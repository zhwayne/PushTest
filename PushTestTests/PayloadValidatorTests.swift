...
final class PayloadValidatorTests: XCTestCase {
    func testInvalidJSONFailsValidation() {
        let errors = PayloadValidator.validate(payloadJSON: "{not-json}", event: .start)
        XCTAssertFalse(errors.isEmpty)
    }

    func testMismatchedEventFailsValidation() {
        let payload = LiveActivityPayloadTemplates.template(for: .update)
        let errors = PayloadValidator.validate(payloadJSON: payload, event: .start)
    ...
    func testUpdateWithValidPayloadPassesValidation() {
        let payload = LiveActivityPayloadTemplates.template(for: .update)
        let errors = PayloadValidator.validate(payloadJSON: payload, event: .update)
    ...
    func testEndRequiresContentState() {
        ...
        let errors = PayloadValidator.validate(payloadJSON: payload, event: .end)
